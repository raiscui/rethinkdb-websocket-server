...
export class Connection {
	...
	start({ sessionCreator, dbHost, dbPort, dbAuthKey, dbSsl }) {
		const urlQueryParams = url.parse(
			this.webSocket.upgradeReq.url,
			true
		).query;
		const req = this.webSocket.upgradeReq;
		this.sessionPromise = sessionCreator(
			urlQueryParams,
			req
		).catch((e) => {
			this.sendWebSocketMessage(
				'rethinkdb-websocket-server session rejected\0'
			);
		...
		this.dbSocket.on('data', (data) => {
			if (!this.isClosed) {
				try {
					...
					this.cleanupAndLogErr(
						'Error recv dbSocket data',
						e
					);
				}
			}
	...
	processClientCommand(cmdBuf) {
		...
		const token =
			cmdBuf.readUInt32LE(0) +
			0x100000000 * cmdBuf.readUInt32LE(4);
		...
		const validatePromise = this.sessionPromise.then((session) =>
			this.queryValidator.validateQueryCmd(
				token,
				queryCmdBuf,
				session,
				logFn
	...
	validateClientHandshake(buf) {
		...
		if (protocolType !== protodef.VersionDummy.Protocol.JSON) {
			this.cleanupAndLogErr(
				'Protocol type not supported ' + protocolType
			);
			return 0;
		}
		return 12;
	...
	handleWsMessage(msg) {
		...
		const incomingBuffer = isBase64
			? new Buffer(msg, 'base64')
			: msg;
		this.wsInBuffer = Buffer.concat([
			this.wsInBuffer,
			incomingBuffer
		]);
		let keepGoing = true;
		while (keepGoing) {
			let bytesConsumed = this.processNextMessage(
				this.wsInBuffer
			);
